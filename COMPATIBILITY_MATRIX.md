...
- Xbox support is limited to the checked-in macOS/browser compatibility evidence
  ...
  reached target BLE `Connected` and Chrome saw deterministic stick, trigger,
  and button movement, but exposed the device as
  `USB2BLE Gamepad (STANDARD GAMEPAD)` rather than an Xbox-like identity.
