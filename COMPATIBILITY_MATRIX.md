...
- Windows single-persona Xbox reconnect/report-delivery fix diagnostic evidence
  ...
  movement.
- Windows single-persona Xbox startup reconnect evidence is checked in at
  `docs/milestone-evidence/WINDOWS_XBOX_STARTUP_RECONNECT_WITNESS_2026-06-09.md`.
  It proves explicit saved startup BLE config can restore the Xbox
  address/persona and XInput deterministic report delivery after target soft
  reset and an operator-assisted ESP32-S3 serial USB power-cycle on Alex's PC,
  without Windows remove/re-pair or serial persona reapply. It does not make
  startup BLE or `persona_static_random_experimental` default, and it does not
  prove durable BLE bond persistence, app/game behavior after reset, broad
  Windows compatibility, or physical HOTAS movement.
